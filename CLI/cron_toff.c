...
int main (void) {
	...
	if (sqlite3_prepare_v2(db, "SELECT localtime, def_mode, options, commands FROM config WHERE id = 1;", -1, &stmt, NULL) != SQLITE_OK) {
	...
	int match = 0;
	snprintf(query, alloc, "SELECT strftime('%%H%%M', time), command FROM rules_turnoff WHERE %s = 1 ORDER BY time(time) ASC;", DAYS[timeinfo -> tm_wday]);
